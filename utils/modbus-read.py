...
# Arguments
if len(sys.argv) < 4:
    print("ARGS: address, functioncode, register, number of decimals")
    sys.exit(1)
slave_addr = int(sys.argv[1])
func_code = int(sys.argv[2])
reg_num = int(sys.argv[3])
...
value = instrument.read_register(reg_num, functioncode=func_code, number_of_decimals=num_decimal, signed=True)
print(value)
